...
const WeatherCard: React.FC<{ forecast: Forecast }> = ({ forecast }) => {
    ...
    return (
        <Card sx={{ minWidth: 250, maxWidth: 400, margin: 3, boxShadow: 3, borderRadius: 2, backgroundColor: '#F0FEF0' }}>
          <CardContent sx={{ textAlign: 'center', padding: 1 }}>
            <Typography variant="subtitle1" component="div" sx={{ fontWeight: 'bold'}}>
              {forecast.dateLabel} （{forecast.date}）
            </Typography>
            <hr style={{ border: 'none', height: '2px', backgroundColor: '#4caf50', width: '90%', margin: 'auto' }} />
            <img src={forecast.image.url} alt={forecast.image.title} style={{ width: '80%', marginTop: 2 }} />
